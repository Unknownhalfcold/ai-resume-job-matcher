...
AI Resume Job Matcher 是一个面向不同专业求职者的简历与岗位匹配分析工具。用户可以上传简历和一份或多份岗位 JD，确认岗位显性与隐性能力，获得可解释的十步综合评分、能力缺口证据和简历优化建议。

<<img width="2880" height="1800" alt="www jobmatcher top_" src="https://github.com/user-attachments/assets/df955f1d-ea55-4f71-83c8-d3b04324ab09" />
>
...
- 输出关键词、语义、经历、简历质量、能力对齐、加分项、惩罚和分数上限。
- 生成岗位能力确认表；用户自评没有证据时只按有限可信度参与评分。
...
- 四阶段响应式 Web 界面：开始、输入、能力确认、结果。
- 本地 Python 分析脚本与 FastAPI API。
- 简历文档解析、多 JD 输入、截图 OCR 和 LLM JD 清洗。
- 跨学科关键词分、岗位能力确认表、LLM 分项评分、十步最终分和结构化 AI 建议。
- 站点默认 LLM 的服务端安全调用链。
...
```text
关键词匹配 → 语义匹配 → 经历匹配 → 简历质量 → 能力对齐
→ 加分项 → 加权基础分 → 核心技能惩罚 → 分数上限 → 最终分数
```
...
能力对齐分由 LLM 结合简历、JD 和用户自评判断，后端再与固定熟练度映射融合。最终总分、惩罚和上限仍由后端函数统一计算。
...
- `/api/capabilities`：每个 IP 每分钟 3 次、24 小时滚动窗口内 20 次。
...
- 从 JD 的明确文本和岗位职责中提取显性、隐性岗位能力，生成用户确认表。
- 判断简历对岗位要求的证据强度、缺口程度和可信度。
- 输出语义匹配、经历匹配、简历质量和能力对齐四个受限分项。
...
线上链路延迟基线和优化判断见 [`docs/PERFORMANCE_REPORT.md`](docs/PERFORMANCE_REPORT.md)。
